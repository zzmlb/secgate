...
| **多节点集中管控** | SSH 连接管理多台服务器，自动检测 SecGate 安装状态和运行端口，一个看板管理所有节点 |
...
将当前服务器作为主节点，通过 SSH 集中管理多台 SecGate 子节点：

#### 节点概览

- **统计卡片** — 节点总数、在线数、离线数、已安装 SecGate 数
- **SecGate 节点网格** — 已安装 SecGate 的节点卡片，显示版本号、Dashboard/Gateway 运行状态，点击可跳转到子节点看板
- **全部节点列表** — 表格展示所有节点，支持测试连接、检测 SecGate、编辑、删除

#### 添加节点

- **三种认证方式** — 密钥文件（自动检测 ~/.ssh/ 下私钥，无需指定路径）、密码、粘贴私钥内容
- **添加前测试** — 先测试 SSH 连接是否通畅，再保存节点
- **编辑节点** — 修改已有节点信息，密码/密钥字段脱敏处理

#### SecGate 检测

- 通过 SSH 远程只读命令检测子节点 SecGate 安装状态
- 5 步检测策略：VERSION 文件 → secgate status → PID 反查端口 → systemd 回退 → curl 端口探测
- 自动发现 Dashboard 和 Gateway 的实际运行端口（不硬编码）
- 所有操作只读，不干预子节点配置
...
│   └── ssh_manager.py   # SSH 连接池 + SecGate 检测
├── scanner/             # 漏洞扫描模块
│   ├── __init__.py      # Flask Blueprint + REST API
│   ├── manager.py       # 扫描任务管理器
