...
基于 Claude + Chainlit 的对话式安全运维助手，通过自然语言交互完成安全分析和问题排查。

**核心能力：**
- **安全状态分析** — 分析 SSH 攻击趋势、查看防火墙拦截情况、检测端口暴露风险
- **网关配置解读** — 查看和解释 IP 白名单、Token 配置、端口保护状态
- **服务异常排查** — 检查 Nginx/iptables/Flask 服务日志，定位故障原因
- **日志分析** — 解析 auth.log、网关日志、看板日志，发现异常模式
- **系统资源监控** — 查看 CPU/内存/磁盘使用情况，排查性能瓶颈
- **安全加固建议** — 根据当前配置主动发现隐患并提供加固方案

**安全机制：**
- 双层认证保护 — 网关层（IP 白名单 + Token）+ Chainlit 层（密码登录，与看板共用账号）
- 只读权限 — 仅允许 Bash 只读命令（cat/grep/tail/ss/ps 等）和文件读取，禁止写入、删除、修改配置
- 工具受限 — 仅开放 Bash、Read、Glob、Grep 四个工具，禁用 Write、Edit 等修改类工具
- 最多 10 轮对话 — 防止长时间占用资源
...
**AI 助手（AI）** — 对话式安全运维
- 用自然语言提问，AI 自动执行命令分析系统状态并给出结论
- 支持安全分析（SSH 攻击、端口暴露、Fail2Ban）、日志排查、资源监控、加固建议
- 只读权限，不会修改系统任何配置，安全无风险
- 与看板共用账号密码，双层认证保护
